...
function main() {
  ...
  screen.key('a', () => {
    ...
    const editor = process.env.EDITOR || process.env.VISUAL || process.env.GIT_EDITOR || (process.platform === 'win32' ? 'notepad' : 'vim');
    const { spawnSync } = require('child_process');
    // temporarily leave blessed fullscreen so editor can use the terminal
    try { screen.leave(); } catch (e) { }
    spawnSync(editor, [taskPath], { stdio: 'inherit', shell: true });
