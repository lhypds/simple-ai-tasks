...
function main() {
  ...
  // completely disable mouse support on Windows to avoid noisy/incompatible events
  const enableMouse = (process.platform !== 'win32');
  const screen = blessed.screen({ smartCSR: true, title: 'stask', fullUnicode: true, mouse: enableMouse });
  ...
  const list = blessed.list({
    ...
    keys: true, vi: true, mouse: enableMouse,
    style: { selected: selectedStyle }
  });

  // If running on Windows, proactively remove any mouse listeners and
  // ensure the mouse option is false so no mouse events are emitted or
  // processed.
  if (process.platform === 'win32') {
    try { if (screen.removeAllListeners) screen.removeAllListeners('mouse'); } catch (e) { }
    try { if (list.removeAllListeners) list.removeAllListeners('mouse'); } catch (e) { }
    try { if (screen.options) screen.options.mouse = false; } catch (e) { }
    try { if (list.options) list.options.mouse = false; } catch (e) { }
  }
