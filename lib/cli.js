...
function openTaskInFs(task) {
  ...
  spawnSync(opener, [path.dirname(task.path)], { stdio: 'ignore' });
}
...
function renderList(screen, list, tasks, selected) {
  const lines = tasks.map((t, i) => {
    ...
    // determine display title: Title -> first 3 chars of Details -> (empty)
    ...
    if (rawTitle) {
      ...
      // take the first letters of the first up to 3 words in Details
      const words = details.split(/\s+/).filter(Boolean);
      if (words.length > 0) {
        title = words.map(w => w[0]).slice(0, 3).join('');
      } else {
...
function main() {
  ...
  const screen = blessed.screen({ smartCSR: true, title: 'stask' });
  ...
  screen.key(['q', 'C-c'], () => {
    ...
    try { process.stdout.write('\x1b[2J\x1b[H'); } catch (e) { console.clear(); }
    process.exit(0);
  });
